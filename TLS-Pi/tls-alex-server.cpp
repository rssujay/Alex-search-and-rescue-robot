...
/* TODO: #define constants for the  filenames for Alex's private key, certificate, CA certificate name,
        ...
        #define PVT_KEY "alex.key"
        #define CERT "alex.crt"
        #define CA_CERT "signing.pem"
        #define COMMON_NAME "laptop.epp.com"
...
void handleCommand(void *conn, const char *buffer)
{
	...
	switch(cmd)
	{
		...
		case 'z':
		case 'Z':
			commandPacket.command = COMMAND_SCAN_COLOUR;
			uartSendPacket(&commandPacket);
			break;
...
int main()
{
	...
	createServer(PVT_KEY, CERT, SERVER_PORT, &worker, CA_CERT, COMMON_NAME, 1);
    /* TODO END */
